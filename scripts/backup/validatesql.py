...
def isValidValuesString(values_string,target_columns):
	...
	lexer = shlex.shlex(values_string, posix=True)
	#lexer.whitespace += ','
	lexer_list = list(lexer)
	
	#lexer_list = shlex.split(values_string)
	
	#print "\n[isValidValuesString][lexer_list] : "
	#print lexer_list
			...
			if current_column[2] == "STRING":
				...
				if (term != ",") and (term != ")"):
					...
					if len(tempString) > current_column[3]:
						...
						return False
					elif (len(tempString) > 0) or (tempString.upper() != "NULL"):
...
def executeInsert(target_columns,values_string):
	global error
	...
	if checkType == "INSERT":
		...
		if isValidSchemaString(schema_string,target_columns):
			if isValidValuesString(values_string,target_columns):
				...
				# Execute INSERT
				executeInsert(target_columns,values_string)
